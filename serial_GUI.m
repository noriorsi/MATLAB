...
% Get T1 positions 
idxT1i = strfind(S, 'T1');
idxT1i = find(not(cellfun('isempty',idxT1i)));
% Get H1 positions 
idxH1i = strfind(S, 'H1');
idxH1i = find(not(cellfun('isempty',idxH1i)));

F0i = zeros(length(idxF0i),1) ;
F1i = zeros(length(idxF1i),1) ;
H1i = zeros(length(idxH1i),1) ;
T1i = zeros(length(idxT1i),1) ;
...
for i = 1:length(idxH1i)
    H1i(i,:) = str2double(S{idxH1i(i)}(5:end)) ;
end

for i = 1:length(idxT1i)
    T1i(i,:) = str2double(S{idxT1i(i)}(5:end)) ;
end
...
plot(F0i)
title(0);
ylabel('Pressure_0 [Hgmm]');
xlabel('Time'); 
title('First Pressure sensor','fontsize',10);
grid

subplot(2,2,2)
plot(F1i)
title(1);
ylabel('Pressure_1 [Hgmm]');
xlabel('Time'); 
title('Second Pressure sensor','fontsize',10);
grid
